...
Функция ТекстПрошлыеВыбытияТоваров() // вт ПрошлыеВыбытияТоваров
	...
	ТекстЗапроса = "
		...
		|
		|	СУММА(ДД.Количество
		|		* ЕСТЬNULL(ПрошлыеПоступления.Количество / ПрошлыеПоступленияБезПартий.Количество, 1)) КАК Количество,
		|	СУММА(ДД.Стоимость
		|		* ЕСТЬNULL(ПрошлыеПоступления.Количество / ПрошлыеПоступленияБезПартий.Количество, 1)) КАК Стоимость,
		|	СУММА(ДД.СтоимостьБезНДС
		|		 * ЕСТЬNULL(ПрошлыеПоступления.Количество / ПрошлыеПоступленияБезПартий.Количество, 1)) КАК СтоимостьБезНДС,
		|	СУММА(ДД.СтоимостьУпр
		|		* ЕСТЬNULL(ПрошлыеПоступления.Количество / ПрошлыеПоступленияБезПартий.Количество, 1)) КАК СтоимостьУпр,
		|	СУММА(ДД.СтоимостьРегл
		|		* ЕСТЬNULL(ПрошлыеПоступления.Количество / ПрошлыеПоступленияБезПартий.Количество, 1)) КАК СтоимостьРегл,
		|	СУММА(ДД.ПостояннаяРазница
		|		* ЕСТЬNULL(ПрошлыеПоступления.Количество / ПрошлыеПоступленияБезПартий.Количество, 1)) КАК ПостояннаяРазница,
		|	СУММА(ДД.ВременнаяРазница
		|		* ЕСТЬNULL(ПрошлыеПоступления.Количество / ПрошлыеПоступленияБезПартий.Количество, 1)) КАК ВременнаяРазница
...
Функция ОписаниеЦепочекПартийНДС(ПараметрыРасчета)
	...
	ДобавитьОписаниеИсточника(ОписаниеЦепочек, Перечисления.ТипыЗаписейПартий.СписаниеНаРасходы,
		Перечисления.ТипыЗаписейПартий.Партия, ПоляПотреблений);
	ДобавитьОписаниеИсточника(ОписаниеЦепочек, Перечисления.ТипыЗаписейПартий.СписаниеНаРасходы,
		Перечисления.ТипыЗаписейПартий.Выпуск, ПоляПотреблений);
