...
Процедура ОбновитьФорматОбменаОрганизацииЕГАИС(КодФСРАР, ФорматОбмена) Экспорт
	...
	ВерсииФорматов =  Новый ТаблицаЗначений;
	ВерсииФорматов.Колонки.Добавить("НомерВерсии", Новый ОписаниеТипов("Число"));
	ВерсииФорматов.Колонки.Добавить("ФорматОбмена", Новый ОписаниеТипов("ПеречислениеСсылка.ФорматыОбменаЕГАИС"));
	
	СтрокаТаблицы = ВерсииФорматов.Добавить();
	СтрокаТаблицы.НомерВерсии  = 1;
	СтрокаТаблицы.ФорматОбмена = Перечисления.ФорматыОбменаЕГАИС.V1;
	
	СтрокаТаблицы = ВерсииФорматов.Добавить();
	СтрокаТаблицы.НомерВерсии  = 2;
	СтрокаТаблицы.ФорматОбмена = Перечисления.ФорматыОбменаЕГАИС.V2;
	
	СтрокаТаблицы = ВерсииФорматов.Добавить();
	СтрокаТаблицы.НомерВерсии  = 3;
	СтрокаТаблицы.ФорматОбмена = Перечисления.ФорматыОбменаЕГАИС.V3;
	
	НомерВерсии = ?(ВерсииФорматов.Найти(ФорматОбмена, "ФорматОбмена") = Неопределено, 0, ВерсииФорматов.Найти(ФорматОбмена, "ФорматОбмена").НомерВерсии);
	
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("Код",            КодФСРАР);
	Запрос.УстановитьПараметр("ВерсииФорматов", ВерсииФорматов);
	Запрос.УстановитьПараметр("НомерВерсии",    НомерВерсии);
	
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ВерсииФорматов.НомерВерсии КАК НомерВерсии,
	|	ВерсииФорматов.ФорматОбмена КАК ФорматОбмена
	|ПОМЕСТИТЬ ВерсииФорматов
	|ИЗ
	|	&ВерсииФорматов КАК ВерсииФорматов
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВерсииФорматов КАК ВерсииФорматов
	|		ПО КлассификаторОрганизацийЕГАИС.ФорматОбмена = ВерсииФорматов.ФорматОбмена
	|ГДЕ
	|	КлассификаторОрганизацийЕГАИС.Код = &Код
	|	И ЕСТЬNULL(ВерсииФорматов.НомерВерсии, 0) < &НомерВерсии";
...
Функция ДокументСоответствуетТипуЕГАИС(ДокументСсылка, ТипЕГАИС)
	...
	Если ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.ТТНВходящаяЕГАИС")
		...
		Возврат ВРег(ТипЕГАИС) = ВРег("WayBill") ИЛИ ВРег(ТипЕГАИС) = ВРег("WayBill_v2") ИЛИ ВРег(ТипЕГАИС) = ВРег("WayBill_v3");
