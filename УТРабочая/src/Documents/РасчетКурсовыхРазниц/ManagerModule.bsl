...
Функция ТребуетсяПереоценка(Организация, Период, РегистрыДляРасчета = Неопределено, АналитикаРасчета = Неопределено) Экспорт
	...
	Если РегистрыДляРасчета.Свойство("РасчетыСПоставщикамиПоДокументам") Тогда
		...
		|ГДЕ
		|	ВЫРАЗИТЬ(Остатки.ДолгОстаток * КурсРегл.Курс / КурсРегл.Кратность / КурсУпр.Курс / КурсУпр.Кратность КАК ЧИСЛО(15,4)) - Остатки.ДолгУпрОстаток >= 0.01
		|	ИЛИ ВЫРАЗИТЬ(Остатки.ДолгОстаток * КурсРегл.Курс / КурсРегл.Кратность / КурсУпр.Курс / КурсУпр.Кратность КАК ЧИСЛО(15,4)) - Остатки.ДолгУпрОстаток <= -0.01
		|	ИЛИ ВЫРАЗИТЬ(Остатки.ДолгОстаток * КурсРегл.Курс / КурсРегл.Кратность КАК ЧИСЛО(15,4)) - Остатки.ДолгРеглОстаток >= 0.01
		|	ИЛИ ВЫРАЗИТЬ(Остатки.ДолгОстаток * КурсРегл.Курс / КурсРегл.Кратность КАК ЧИСЛО(15,4)) - Остатки.ДолгРеглОстаток <= -0.01
		|	ИЛИ (Остатки.ПредоплатаОстаток = 0 И Остатки.КВозвратуОстаток = 0
		|		И (ВЫРАЗИТЬ(Остатки.ПредоплатаОстаток * КурсРегл.Курс / КурсРегл.Кратность КАК ЧИСЛО(15,4)) - Остатки.ПредоплатаРеглОстаток >= 0.01
		|			ИЛИ ВЫРАЗИТЬ(Остатки.ПредоплатаОстаток * КурсРегл.Курс / КурсРегл.Кратность КАК ЧИСЛО(15,4)) - Остатки.ПредоплатаРеглОстаток <= -0.01
		|			ИЛИ ВЫРАЗИТЬ(Остатки.ПредоплатаОстаток * КурсРегл.Курс / КурсРегл.Кратность / КурсУпр.Курс / КурсУпр.Кратность КАК ЧИСЛО(15,4)) - Остатки.ПредоплатаУпрОстаток >= 0.01
		|			ИЛИ ВЫРАЗИТЬ(Остатки.ПредоплатаОстаток * КурсРегл.Курс / КурсРегл.Кратность / КурсУпр.Курс / КурсУпр.Кратность КАК ЧИСЛО(15,4)) - Остатки.ПредоплатаУпрОстаток <= -0.01
		|			)
		|		)";
	КонецЕсли;
	...
	Если РегистрыДляРасчета.Свойство("ДенежныеСредстваБезналичные") Тогда
		...
		|	ПО &ВалютаУпрУчета = КурсУпр.Валюта
		|ГДЕ
		|	ВЫРАЗИТЬ(Остатки.СуммаОстаток * КурсРегл.Курс / КурсРегл.Кратность / КурсУпр.Курс / КурсУпр.Кратность КАК ЧИСЛО(15,4)) - Остатки.СуммаУпрОстаток >= 0.01
		|	ИЛИ ВЫРАЗИТЬ(Остатки.СуммаОстаток * КурсРегл.Курс / КурсРегл.Кратность / КурсУпр.Курс / КурсУпр.Кратность КАК ЧИСЛО(15,4)) - Остатки.СуммаУпрОстаток <= -0.01
		|	ИЛИ ВЫРАЗИТЬ(Остатки.СуммаОстаток * КурсРегл.Курс / КурсРегл.Кратность КАК ЧИСЛО(15,4)) - Остатки.СуммаРеглОстаток >= 0.01
		|	ИЛИ ВЫРАЗИТЬ(Остатки.СуммаОстаток * КурсРегл.Курс / КурсРегл.Кратность КАК ЧИСЛО(15,4)) - Остатки.СуммаРеглОстаток <= -0.01";
	КонецЕсли;
	...
	Если РегистрыДляРасчета.Свойство("РасчетыПоДоговорамКредитовИДепозитов") Тогда
		...
		|	ПО &ВалютаУпрУчета = КурсУпр.Валюта
		|ГДЕ
		|	ВЫРАЗИТЬ(Остатки.СуммаВВалютеОстаток * КурсРегл.Курс / КурсРегл.Кратность / КурсУпр.Курс / КурсУпр.Кратность КАК ЧИСЛО(15,4)) - Остатки.СуммаУпрОстаток >= 0.01
		|	ИЛИ ВЫРАЗИТЬ(Остатки.СуммаВВалютеОстаток * КурсРегл.Курс / КурсРегл.Кратность / КурсУпр.Курс / КурсУпр.Кратность КАК ЧИСЛО(15,4)) - Остатки.СуммаУпрОстаток <= -0.01
		|	ИЛИ ВЫРАЗИТЬ(Остатки.СуммаВВалютеОстаток * КурсРегл.Курс / КурсРегл.Кратность КАК ЧИСЛО(15,4)) - Остатки.СуммаРеглОстаток >= 0.01
		|	ИЛИ ВЫРАЗИТЬ(Остатки.СуммаВВалютеОстаток * КурсРегл.Курс / КурсРегл.Кратность КАК ЧИСЛО(15,4)) - Остатки.СуммаРеглОстаток <= -0.01";
	КонецЕсли;
